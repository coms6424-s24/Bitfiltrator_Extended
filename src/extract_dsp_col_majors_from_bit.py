...
def get_differing_major_columns(
  ...
  idcode_majorRow_majorCol = list(
    dict.fromkeys(
      [(idcode, far.half_bit,far.row_addr, far.col_addr) for (idcode, far) in diff_idcodeFar]
    )
  )
...
def get_majors(
  ...
  diff_slrNameMajorRow_majorCols: dict[tuple[str, int, int], list[int]] = defaultdict(list)
  for (idcode, major_row, half_bit, major_col) in diff_idcode_majorRow_majorCol:
    ...
    if not is_clb_column:
      diff_slrNameMajorRow_majorCols[(slr_name, half_bit, major_row)].append(major_col)
  ...
  for ((slr_name,half_bit, major_row), major_cols) in diff_slrNameMajorRow_majorCols.items():
    num_minors_per_std_colMajor = device_summary.get_num_minors_per_std_col_major(slr_name, half_bit,major_row)
  ...
  for ((slr_name,half_bit, major_row), major_cols) in diff_slrNameMajorRow_majorCols.items():
    ...
    for major_col in major_cols:
      ...
      if num_minors < max_num_minors:
        slrName_majorRow_dspMajorCols[(slr_name,half_bit, major_row)].append(major_col)
  ...
  for (slr_name, cr_row_idx, cr_dsp_bottom, cr_dsp_top) in dsps:
    ...
    slrNameMajorRow_dspXs[(slr_name, half_bit, major_row)].append(dsp_bottom_x)
    slrNameMajorRow_dspYBoundaries[(slr_name,half_bit, major_row)] = (dsp_bottom_y, dsp_top_y)
  ...
  for (slr_name, half_bit, major_row) in slrName_majorRow_dspMajorCols:
    dsp_majorCols = slrName_majorRow_dspMajorCols[(slr_name, half_bit, major_row)]
    dsp_Xs = slrNameMajorRow_dspXs[(slr_name, half_bit, major_row)]
    (dsp_bottom_y, dsp_top_y) = slrNameMajorRow_dspYBoundaries[(slr_name, half_bit, major_row)]
    # print(f"(slr, row) = ({slr_name}, {major_row})")
    # print(f"dsp_majorCols = {dsp_majorCols}")
    # print(f"dsp_Xs        = {dsp_Xs}")
    assert len(dsp_majorCols) == len(dsp_Xs), f"Error: Expected the same number of DSP major columns as the number of DSPs in half {half_bit} in relative major row {major_row} in {slr_name}"

    res["slrs"][slr_name]["DSP"]["half_bit"][str(half_bit)]["rowMajors"][str(major_row)]["min_dsp_y_ofst"] = dsp_bottom_y
    res["slrs"][slr_name]["DSP"]["half_bit"][str(half_bit)]["rowMajors"][str(major_row)]["max_dsp_y_ofst"] = dsp_top_y
    for (dsp_majorCol, dsp_x) in zip(dsp_majorCols, dsp_Xs):
      res["slrs"][slr_name]["DSP"]["half_bit"][str(half_bit)]["rowMajors"][str(major_row)]["colMajors"][str(dsp_x)] = dsp_majorCol
