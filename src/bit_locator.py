...
#################
#ASSUMING BEL_X AND BEL_Y ARE ABSOLUTE
# ASSUMING get_min_clock_region_row_idx (and the max version) ARE ABSOLUTE ROWS, OR CANT TELL BETWEEN SLRs
#ASSUMING get_min_row_majors (and the max version) are ABSOLUTE
import helpers
import resources
...
class BitLocator:
  ...
  def _get_slr_name(
    ...
    slr_absolute_minMaxY: dict[str, tuple[int, int]] = dict()
    for slr_name in self._device_summary.get_slr_names():
      (min_absolute_rowMajor, max_absolute_rowMajor) = self._get_min_max_row_majors_in_slr(slr_name)
      min_y = min_absolute_rowMajor * num_bel_per_column
      max_y = (max_absolute_rowMajor + 1) * num_bel_per_column - 1
      #min_y =0
      #max_y=(max_absolute_rowMajor + 1) * num_bel_per_column - 1 - min_absolute_rowMajor * num_bel_per_column
      slr_absolute_minMaxY[slr_name] = (min_y, max_y)

    # Locate which SLR the BEL is in.
    target_slrName = None
    for (slr_name, (min_y, max_y)) in slr_absolute_minMaxY.items():
