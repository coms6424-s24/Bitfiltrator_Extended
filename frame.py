...
#assuming the get_min_far_row_idx and get_max_far_row_idx are relative to each half
#changing inputs to get_num_minors_per_std_col_major (slrName, half_bit rowMajor)
#and get_num_minors_per_bram_content_col_major(slrName, half_bit, rowMajor)
...
class FrameAddressRegister:
  ...
  def to_bin(self) -> str:
    ...
    b_half_bit = f"{self.block_type.value:0>{self.spec.far_half_bit_width()}b}"
    ...
    b_far = f"{b_reserved}{b_block_type}{b_half_bit}{b_row_addr}{b_col_addr}{b_minor_addr}"
    return b_far
  ...
  def __eq__(self, other) -> bool:
    self_keys = (self.reserved, self.block_type, self.half_bit, self.row_addr, self.col_addr, self.minor_addr)
    other_keys = (other.reserved, other.block_type, other.half_bit, other.row_addr, other.col_addr, other.minor_addr)
  ...
  def from_int(
    ...
    half_bit = helpers.bits(far, spec.far_half_bit_idx_high(), spec.far_half_bit_idx_low())
    ...
    return FrameAddressRegister(reserved, block_type, half_bit, row_addr, col_addr, minor_addr, spec)

  def __str__(self) -> str:
    return f"BLOCK_TYPE = {self.block_type.name}, half_bit = {self.half_bit:>3d},  ROW_ADDR = {self.row_addr:>3d}, COL_ADDR = {self.col_addr:>3d}, MINOR_ADDR = {self.minor_addr:>3d}"
...
class FrameAddressIncrementer:
  def __init__(
    ...
    self.num_minors_per_bram_colMajor: dict[
      int, # IDCODE
      int, #bottom_half (i.e. half_bit=0)
      dict[
        int, # row major
        list[int] # num minors per column
      ],
      int, #top_half (i.e. half_bit=1)
      dict[
        int, # row major
        list[int] # num minors per column
      ]
    ...
    for slrName in device_summary.get_slr_names():
      ...
      min_rowMajor = device_summary.get_min_far_row_idx(slrName) #assume RELATIVE
  ...
  def increment(
   ...
   # Carry over row major to half and then to block type.
    ...
    return FrameAddressRegister(far.reserved, new_block_type,new_half, new_row_addr, new_col_addr, new_minor_addr, far.spec)
